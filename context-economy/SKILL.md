...
## When this skill makes things worse

Every rule above optimises the cheap resource. Tokens are cheap; a wrong answer is not. These
are the cases where following the skill costs more than ignoring it.
...
- **`/clear` throws away context that had value.** Re-establishing it costs tokens and your time.
  Clear on a genuine change of subject, not on a tidy impulse.
- **On a flat-rate seat, only the accuracy argument applies.** Do not spend real effort chasing
  tokens that are not billed. Spend it on keeping distractors out.
