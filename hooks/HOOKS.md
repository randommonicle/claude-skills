...
| `secret-echo-guard.mjs` | PreToolUse / Bash\|PowerShell, and Read | a command whose OUTPUT would carry a secret value → `permissionDecision: "deny"` with the safe form in the reason (mechanises secrets-in-output). Six shapes, each one that prints a value and never one that only uses it: a fallback expansion of a secret-named variable inside a print (`${X:-word}`, the indirect `${!v:-word}` loop); a print of one (`echo $X`, `Write-Host $env:X`, `console.log(process.env.X)`, a heredoc to stdout, a bare PowerShell expression); an environment listing bare, wildcarded or naming a secret (`printenv`, `env \| grep`, `set`, `declare -p X`, `Get-ChildItem Env:`) unless piped into a names-only sink (`cut -d= -f1`, `Select-Object Name`); a `.env` file read whole or grepped without `-c/-l/-q` or a keys-only `-o` (the `.example`/`.sample`/`.template` variants pass); `supabase status` not captured into `$(...)`/a file and not filtered of key lines; `curl -v` on a request carrying `Authorization`/`apikey`; and the Read tool on a `.env` file. Secret-named = a `KEY`/`TOKEN`/`SECRET`/`PASSWORD`/`PASS`/`CREDENTIAL`/`PRIVATE`/`AUTH`/`COOKIE`/`SESSION`/`DSN` segment or a `DATABASE_URL`, minus `PUBLIC`/`PUBLISHABLE`/`ANON` and `_SOCK`. Presence and length (`${#X}`, `[ -n "$X" ]`, `$env:X.Length`), assignments and a secret merely USED (a curl header, an env prefix, a script reading `process.env`) stay silent. A deny, not an ask, from the first day: in a bypass-permissions session an ask is auto-approved and shown to nobody, and both incidents were routine checks by an agent (2026-09-18, a Sonnet agent's bare `npx supabase status`; 2026-09-19, the main session's `${X:-unset}` loop, ICC L-045), so the warn-first path was already spent | fail-closed for matched commands (the reason carries the rewrite, so a false positive costs one retry); exits 0 on script error. Residual: a value copied into another variable and printed, a script printing `process.env` from its own source, and an API response returning a credential are not command-string shapes (skill is the backstop) |
