...
| `secret-echo-guard.mjs` | PreToolUse / Bash\|PowerShell, and Read | a command whose OUTPUT would carry a secret value → `permissionDecision: "deny"` with the safe form in the reason (mechanises secrets-in-output). Reads the command with a quote-aware scanner (single quotes literal; double quotes and `@"..."@` expanding; backslash, or the backtick under the PowerShell tool, escaping; `$(`/backtick/`<(` depth; `${}` braces), so a `;` or `\|` inside a commit message is text and a `'$X'` is a literal. Six shapes, each one that prints a value and never one that only uses it: a fallback or indirect expansion of a secret-named variable inside a print; a print of one (echo, printf, the Write-*/Out-*/Format-* cmdlets, console.log, process.stdout.write, print(), a heredoc or here-string to a printing filter, a bare PowerShell expression, `set -x` tracing), judged per occurrence with only `${#X}`, a `${X:+word}` without a `$` and `$env:X.Length` safe inside a print, and a stdout that reaches a file or a digest/count consumer not a print; an environment listing (printenv, env, set, declare -p, Get-ChildItem Env:, Get-Variable, GetEnvironmentVariable, /proc/*/environ) unless piped into a field-exact names-only sink with no tee before it; a secret file (`.env*`, `*.env`, `.envrc`, `*.pem`, `id_rsa`, `*.key`, `.credentials.json`, `.npmrc`, `.netrc`, `.pgpass`, ...) read whole by a reader or encoder, or grepped without a quiet flag of its own or keys-only `-o` (quiet flags are read from that grep's own pipe segment and per tool, so a later `head -c` is not grep's, `rg -L` follows symlinks, Select-String counts only `-Quiet`, and a grep feeding a quiet grep passes; every grep in the statement is judged, 2026-09-24; the `.example`/`.sample`/`.template`/`.dist` variants pass; a grep's pattern is text, not a file: only a token proven to be the pattern is exempt, so a secret file as an operand, an option's value, via xargs or `<`, or after an option the parser does not know still counts, 2026-09-24); a provider command that prints secrets (supabase status, aws ssm --with-decryption and secretsmanager, az keyvault, gcloud secrets, vault, op, doppler, netlify env, heroku config, stripe config) in command position at any depth unless captured, projected or publishable; `curl -v` with an auth header. Redirects are read left to right (`1>file` captures, a later `>&2` undoes it); wrappers (`sh -c`/`-xc`, `pwsh -Command`, `cmd /c`, `eval`, `Invoke-Expression`, `sudo`/`env` prefixes) are analysed inside. A deny, not an ask, from the first day (DECISIONS.md 2026-09-20): both incidents were routine checks by an agent and an ask is invisible in a bypass-permissions session. Pattern set attacked in a two-round cross-agent review (GPT via codex, 2026-09-20): 76 false negatives and 21 false positives reproduced and fixed, all in the sibling suite (199 cases) | fail-closed for matched commands (the reason carries the rewrite, so a false positive costs one retry); exits 0 on script error; about 90-100 ms per call at 100 KB and about 320 ms on a 1 MB `echo`, node start included (re-measured 2026-09-24 with that day's rule 4 changes, median of seven; the previous hook measured 94-98 ms and 279 ms on the same inputs). No override switch: removing its two `settings.json` entries is the off. Residual, by design: a routine-slip guard judged from the command string, not an exfiltration boundary. A value copied into another variable and printed, any script file run by name (its contents are never read, so an `echo $KEY` inside `bash phase.sh` passes, measured 2026-09-24), a recursive grep over a folder holding a secret file (`grep -rn TOKEN .`: only a file named in the command counts), a secret under an unrecognised name, an encoder the reader set does not name, an API response returning a credential, and the `Grep` tool over `.env` (only `Read` is gated) all pass: the skill is the control for them. Known false positives: fixture files named `credentials.json` or `*.key`, and a grep pattern after an option the parser's tables do not list (`rg --no-config "process.env" src`; the short-option tables match the --help of GNU grep 3.0 (plus its obsolete -y) and of ripgrep 14.1.1, the long ones cover only the common options) |
| `session-recon.mjs` | SessionStart | in a git repo: fetch, `status -sb`, all-refs log, open PRs → injected as `additionalContext` (parallel-work-recon's session-start half). Also reports this library's own update health from `../skills-update.json`, read BEFORE the repo check so it reaches a session opened outside a repo: silent while the state is `current`, one line when the updater refused or errored, when a fast-forward changed `hooks/` or a `SKILL.md` (so this session is the first to load them), or when no run has completed in 36h — a scheduled task that stopped is the failure that last branch exists to make visible. On a direct-clone machine (the library at `<config>/skills`, no `CLAUDE_PLUGIN_ROOT`) it also compares the norm block in `<config>/CLAUDE.md` with `NORMS.md`, line endings aside: silent when they match, and one line to the model plus a `systemMessage` to the person when the file is missing, the block is missing, or its text or marker differs (2026-09-24, LESSONS_LEARNED 23); a plugin install or any other layout is skipped | fail-open, silent on timeout/offline; a MISSING status file is silence by design (the scheduler is per-machine and may never have been installed), a STALE one is not |
  ...
  "hooks": {
    ...
    "PreToolUse": [
      { "matcher": "Bash|PowerShell", "hooks": [
        ...
        { "type": "command", "command": "node \"$USERPROFILE/.claude/skills/hooks/secret-echo-guard.mjs\"", "shell": "bash" }
      ] },
      { "matcher": "Read", "hooks": [
        { "type": "command", "command": "node \"$USERPROFILE/.claude/skills/hooks/secret-echo-guard.mjs\"", "shell": "bash" }
      ] },
